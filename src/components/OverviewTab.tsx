  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm font-extrabold text-foreground mt-0.5 flex items-center gap-1.5">
                    ...
                    <span className="text-[10px] font-mono text-muted-foreground">({selectedCity.windDir})</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Right Section: Actuators & Alerts Log */}
        <section className="lg:col-span-4 space-y-6">
          
          {/* Actuation Control Panel */}
          <div className="bg-card border border-border rounded-xl p-5 md:p-6 shadow-sm flex flex-col h-fit">
            <div className="flex items-center gap-2 border-b border-border/60 pb-4 mb-4">
              <Zap className="h-5 w-5 text-blue-500" />
              <div>
                <h3 className="font-bold text-sm text-foreground">Actuation Command</h3>
                <span className="text-[10px] text-muted-foreground font-mono">Mitigation Overrides</span>
              </div>
            </div>
            ...
            <div className={`space-y-3.5 ${isLiveSync ? "opacity-60 cursor-not-allowed" : ""}`}>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    selectedCity.actuators.mistCannons ? "bg-blue-500/10 text-blue-500" : "bg-muted text-muted-foreground"
                  }`}>
                    <RotateCw className={`h-4.5 w-4.5 ${selectedCity.actuators.mistCannons ? "animate-[spin_4s_linear_infinite]" : ""}`} />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold leading-snug">Mist Cannons & Smog Towers</h4>
                    <p className="text-[9px] text-muted-foreground leading-normal mt-0.5">Washes down PM2.5 / PM10</p>
                  </div>
                </div>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${
                  selectedCity.actuators.mistCannons ? "bg-blue-500" : "bg-zinc-300 dark:bg-zinc-700"
                }`}>
                  <div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${
                    selectedCity.actuators.mistCannons ? "translate-x-4" : "translate-x-0"
                  }`} />
                </div>
              </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    selectedCity.actuators.trafficRedirect ? "bg-indigo-500/10 text-indigo-500" : "bg-muted text-muted-foreground"
                  }`}>
                    <Car className={`h-4.5 w-4.5 ${selectedCity.actuators.trafficRedirect ? "animate-pulse" : ""}`} />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold leading-snug">Green Corridor Diversions</h4>
                    <p className="text-[9px] text-muted-foreground leading-normal mt-0.5">Reduces localized NO2 / CO</p>
                  </div>
                </div>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${
                  selectedCity.actuators.trafficRedirect ? "bg-indigo-500" : "bg-zinc-300 dark:bg-zinc-700"
                }`}>
                  <div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${
                    selectedCity.actuators.trafficRedirect ? "translate-x-4" : "translate-x-0"
                  }`} />
                </div>
              </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    selectedCity.actuators.industrialCap ? "bg-rose-500/10 text-rose-500" : "bg-muted text-muted-foreground"
                  }`}>
                    <Factory className={`h-4.5 w-4.5 ${selectedCity.actuators.industrialCap ? "animate-pulse" : ""}`} />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold leading-snug">Clean-Air Industrial Cap</h4>
                    <p className="text-[9px] text-muted-foreground leading-normal mt-0.5">Damps factory SO2 / PM2.5</p>
                  </div>
                </div>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${
                  selectedCity.actuators.industrialCap ? "bg-rose-500" : "bg-zinc-300 dark:bg-zinc-700"
                }`}>
                  <div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${
                    selectedCity.actuators.industrialCap ? "translate-x-4" : "translate-x-0"
                  }`} />
                </div>
              </div>
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg transition-colors ${
                    selectedCity.actuators.publicBroadcast ? "bg-amber-500/10 text-amber-500" : "bg-muted text-muted-foreground"
                  }`}>
                    <Radio className={`h-4.5 w-4.5 ${selectedCity.actuators.publicBroadcast ? "animate-[bounce_1.5s_infinite]" : ""}`} />
                  </div>
                  <div>
                    <h4 className="text-xs font-bold leading-snug">Emergency Public Advisory</h4>
                    <p className="text-[9px] text-muted-foreground leading-normal mt-0.5">citizen warning notifications</p>
                  </div>
                </div>
                <div className={`w-8 h-4 rounded-full relative transition-colors ${
                  selectedCity.actuators.publicBroadcast ? "bg-amber-500" : "bg-zinc-300 dark:bg-zinc-700"
                }`}>
                  <div className={`absolute top-0.5 left-0.5 w-3 h-3 rounded-full bg-white transition-transform ${
                    selectedCity.actuators.publicBroadcast ? "translate-x-4" : "translate-x-0"
                  }`} />
                </div>
              </div>
            </div>

            {/* Mitigations summary block */}
            <div className="mt-5 p-3 rounded-lg bg-muted border border-border/80 text-[10px] text-muted-foreground font-mono space-y-1">
              <span className="font-bold text-foreground block">Actuation Impact Log:</span>
              {Object.values(selectedCity.actuators).some(Boolean) ? (
                <ul className="list-disc pl-3.5 space-y-0.5">
                  {selectedCity.actuators.mistCannons && <li className="text-blue-500">Atomized mist reduces PM2.5 (~22%) & PM10 (~35%)</li>}
                  {selectedCity.actuators.trafficRedirect && <li className="text-indigo-500">Green corridor redirection bypass reduces NO2 (~25%) & CO (~30%)</li>}
                  {selectedCity.actuators.industrialCap && <li className="text-rose-500">Power cap limits factory emissions: SO2 (~38%) & PM2.5 (~12%)</li>}
                  {selectedCity.actuators.publicBroadcast && <li className="text-amber-500">Advisory: citizen activities and trash burning reduced (~4%)</li>}
                </ul>
              ) : (
                <p>No active mitigations. Telemetry reflecting base municipal emissions.</p>
              )}
            </div>
          </div>

          {/* Geographical Alerts Log */}
          <div className="bg-card border border-border rounded-xl p-5 md:p-6 shadow-sm flex flex-col h-[320px]">
            <div className="flex items-center justify-between border-b border-border/60 pb-4 mb-4">
              <div className="flex items-center gap-2">
                <BellRing className="h-5 w-5 text-rose-500 animate-pulse" />
                <div>
                  <h3 className="font-bold text-sm text-foreground">Alerts & System Logs</h3>
                  <span className="text-[10px] text-muted-foreground font-mono">Geographical Feed</span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-1.5 py-0.5 rounded bg-muted hover:bg-accent border border-border text-[9px] font-mono text-muted-foreground hover:text-foreground select-none"
                ...
                <button 
                  ...
                  className="p-1 rounded bg-muted hover:bg-rose-500/10 hover:text-rose-500 border border-border text-[9px]"
                  title="Clear feed"
                >
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-1 font-mono text-[10px] scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
              {alerts.length > 0 ? (
                alerts.map((alert) => (
                  <div 
                    key={alert.id} 
                    className={`p-2.5 rounded border text-[10px] transition-all ${
                      alert.level === "critical" 
                        ...
                        : alert.level === "warning" 
                          ...
                          : "bg-zinc-500/5 border-border text-muted-foreground"
                    }`}
                  >
                    <div className="flex items-center justify-between mb-1 text-[9px] font-semibold opacity-80">
                ...
                <div className="h-full flex flex-col items-center justify-center text-muted-foreground text-center select-none py-12">
                  <CheckCircle2 className="h-8 w-8 text-emerald-500/40 mb-2" />
                  <p>All Nodes Safe.</p>
                  <p className="text-[9px] mt-0.5">No violations active on the network.</p>
                </div>
              )}
            </div>
          </div>
